...
function getFinalExportState(){
  return {
    locked: localStorage.getItem(STORAGE_PREFIX + "finalExportLocked") === "1",
    finalExportText: localStorage.getItem(STORAGE_PREFIX + "finalExportText") || "",
    finalExportIso: localStorage.getItem(STORAGE_PREFIX + "finalExportIso") || "",
    finalExportHash: localStorage.getItem(STORAGE_PREFIX + "finalExportHash") || "",
    finalExportStatus: localStorage.getItem(STORAGE_PREFIX + "finalExportStatus") || ""
  };
}
function setPrintableFinalExport(){
  const st = getFinalExportState();
  const finalTimePrint = qs("#finalExportTimePrint");
  const finalStatusPrint = qs("#finalExportStatusPrint");
  const finalHashPrint = qs("#finalAnswerHashPrint");
  const finalBanner = qs("#finalLockBanner");
  if(finalTimePrint) finalTimePrint.innerHTML = st.locked ? `<span class="code-access-print">${st.finalExportText || "Heure non enregistrée"}</span>` : "Export final non effectué";
  if(finalStatusPrint) finalStatusPrint.innerHTML = st.locked ? `<span class="locked-identity-badge">copie finale verrouillée</span>` : "Copie non encore verrouillée";
  if(finalHashPrint) finalHashPrint.innerHTML = st.locked ? `<span class="code-access-print verification-fingerprint">${st.finalExportHash || "Empreinte non générée"}</span>` : "Empreinte non générée";
  if(finalBanner){
    finalBanner.classList.toggle("hidden", !st.locked);
    if(st.locked){
      finalBanner.innerHTML = `<strong>Copie finale verrouillée</strong> — export final : ${st.finalExportText || "heure non enregistrée"} — aucune modification n’est autorisée.`;
    }
  }
}

function isFinalLocked(){
  return localStorage.getItem(STORAGE_PREFIX + "finalExportLocked") === "1";
}
function prepareFinalLockedGate(){
  const gate = qs("#accessGate");
  if(!gate) return;
  const st = getFinalExportState();
  const ident = getLockedIdentity();
  gate.classList.remove("hidden");
  gate.classList.add("final-submission-gate", "screen-only");
  gate.innerHTML = `
    <div class="overlay-card final-submission-card">
      <h2>Copie finale déjà verrouillée</h2>
      <p><strong>Cette remise finale a déjà été exportée.</strong></p>
      <p>Il n’est plus permis de modifier les réponses, les cases à cocher ou l’identité.</p>
      <div class="final-receipt">
        <div><strong>Nom validé :</strong> ${ident.studentName || "—"}</div>
        <div><strong>N° étudiant :</strong> ${ident.studentId || "—"}</div>
        <div><strong>Groupe :</strong> ${ident.studentGroup || "—"}</div>
        <div><strong>Code :</strong> ${ident.code || "—"}</div>
        <div><strong>Activation :</strong> ${ident.time || "—"}</div>
        <div><strong>Export final :</strong> ${st.finalExportText || "—"}</div>
        <div><strong>Session :</strong> ${ident.sessionId || "—"}</div>
        <div><strong>Empreinte finale :</strong> ${st.finalExportHash || "—"}</div>
      </div>
      <p class="small muted">Le bouton ci-dessous réexporte uniquement la copie finale verrouillée déjà enregistrée localement. Aucune reprise de l’examen n’est autorisée.</p>
      <div class="actions">
        <button onclick="exportPdf()">Réexporter la copie finale verrouillée</button>
      </div>
    </div>`;
}
function enforceFinalLockHard(showGate=false){
  if(!isFinalLocked()) return false;
  restoreFinalSnapshot();
  lockExamAfterFinalExport();
  document.body.classList.add("submitted-final");
  examStarted = false;
  clearInterval(timerId);
  clearInterval(fsCountdownId);
  const fsOverlay = qs("#fullscreenLock");
  if(fsOverlay) fsOverlay.classList.add("hidden");
  document.body.classList.remove("fs-locked");
  updateAccessDisplay();
  if(showGate) prepareFinalLockedGate();
  return true;
}

function collectAnswerSnapshot(){
  const ident = getLockedIdentity();
  const data = {
    version: VERIFICATION_SOURCE,
    identity: ident,
    finalExportIso: localStorage.getItem(STORAGE_PREFIX + "finalExportIso") || "",
    fields: {},
    radios: {},
  ...
  qsa("input,textarea,select").forEach(el=>{
    if(el.closest("#accessGate")) return;
    const key = el.name || el.id;
    if(!key) return;
    if(el.type === "radio"){
      if(el.checked) data.radios[key] = el.value;
...
function saveFinalSnapshot(snapshot){
  localStorage.setItem(STORAGE_PREFIX + "finalSnapshot", JSON.stringify(snapshot));
}
function restoreFinalSnapshot(){
  const raw = localStorage.getItem(STORAGE_PREFIX + "finalSnapshot");
  if(!raw) return;
  try{
    const snap = JSON.parse(raw);
    Object.entries(snap.fields || {}).forEach(([key, value])=>{
      const el = qs(`[name="${CSS.escape(key)}"], #${CSS.escape(key)}`);
      if(!el) return;
      if(el.type === "checkbox") el.checked = !!value;
      else el.value = value;
    });
    Object.entries(snap.radios || {}).forEach(([name, value])=>{
      const el = qs(`input[type="radio"][name="${CSS.escape(name)}"][value="${CSS.escape(value)}"]`);
      if(el) el.checked = true;
    });
    qsa("textarea").forEach(autoResize);
  }catch(e){ console.warn("Snapshot final impossible à restaurer", e); }
}
function lockExamAfterFinalExport(){
  restoreFinalSnapshot();
  qsa("input, textarea, select").forEach(el=>{
    if(el.closest("#accessGate")) return;
    el.readOnly = true;
    if(el.type === "radio" || el.type === "checkbox" || el.tagName === "SELECT") el.disabled = true;
    el.classList.add("final-locked-field");
  });
  document.body.classList.add("final-export-locked");
  setPrintableFinalExport();
}
async function markFinalExportInFirebase(finalExportIso, finalExportText, finalHash){
  const ident = getLockedIdentity();
  if(!ident.sessionId) return;
  try{
    await updateDoc(doc(db, "examens", EXAM_ID, "accessLogs", ident.sessionId), {
      exportFinalAt: serverTimestamp(),
      exportFinalClientIso: finalExportIso,
      exportFinalClientText: finalExportText,
      finalStatus: "locked_after_pdf_export",
      finalAnswerHash: finalHash
    });
  }catch(e){
    console.warn("Trace Firebase de l’export final non enregistrée. Le PDF reste exportable localement.", e);
  }
}
...
function prepareResumeGate(){
  if(isFinalLocked()){ enforceFinalLockHard(true); return; }
  const gate = qs("#accessGate");
  if(!gate) return;
  gate.classList.remove("hidden");
  gate.innerHTML = `
...
async function validateAccess(){
  if(enforceFinalLockHard(true)) return;
...
["beforeinput","input","change","keydown","paste","cut","drop"].forEach(evt=>{
  document.addEventListener(evt, e=>{
    if(!isFinalLocked()) return;
    if(e.target && e.target.closest && e.target.closest("#accessGate")) return;
    if(e.target && e.target.closest && e.target.closest(".top-monitor")) return;
    e.preventDefault();
...
window.addEventListener("pageshow", ()=>{ if(isFinalLocked()) enforceFinalLockHard(true); });
window.addEventListener("focus", ()=>{ if(isFinalLocked()) enforceFinalLockHard(true); });
document.addEventListener("visibilitychange", ()=>{ if(!document.hidden && isFinalLocked()) enforceFinalLockHard(true); });
window.addEventListener("afterprint", ()=>{ if(isFinalLocked()) enforceFinalLockHard(true); });
...
async function exportPdf(){
  ...
  if(!getLockedIdentity().code){
    alert("Vous devez d’abord valider votre code d’accès avant d’exporter le PDF.");
    return;
  }

  if(getFinalExportState().locked){
    alert("Cette copie est déjà verrouillée comme remise finale. Le PDF réexporté doit être identique à la copie finale enregistrée.");
    restoreFinalSnapshot();
    lockExamAfterFinalExport();
    updateAccessDisplay();
    window.print();
  ...
  const confirmation = confirm(
    "Attention : l’export PDF est considéré comme la remise finale de votre examen.\n\n" +
    "Après confirmation, vos réponses seront verrouillées. Vous ne pourrez plus modifier l’examen ni produire une nouvelle version différente.\n\n" +
    "Avant de continuer, vérifiez vos réponses. Voulez-vous confirmer l’export final ?"
  );
  if(!confirmation) return;

  const finalExportIso = new Date().toISOString();
  const finalExportText = nowString();
  localStorage.setItem(STORAGE_PREFIX + "finalExportIso", finalExportIso);
  localStorage.setItem(STORAGE_PREFIX + "finalExportText", finalExportText);
  localStorage.setItem(STORAGE_PREFIX + "finalExportStatus", "locked_after_pdf_export");

  const snapshot = collectAnswerSnapshot();
  const finalHash = formatFingerprint(await sha256Hex(JSON.stringify(snapshot)));
  localStorage.setItem(STORAGE_PREFIX + "finalExportHash", finalHash);
  localStorage.setItem(STORAGE_PREFIX + "finalExportLocked", "1");
  snapshot.finalExportIso = finalExportIso;
  snapshot.finalExportText = finalExportText;
  snapshot.finalAnswerHash = finalHash;
...
function resetLocalExam(){
  if(isFinalLocked()){ alert("La copie finale est verrouillée. L’effacement local est désactivé pour cette remise finale."); return; }
...
window.addEventListener("DOMContentLoaded", ()=>{
  ...
  updateAccessDisplay();
  lockIdentityFields();
  setInterval(()=>{ enforceLockedIdentity(); if(isFinalLocked()) enforceFinalLockHard(false); }, 1500);
  qsa("input,textarea,select").forEach(el=>{
  ...
  if(isFinalLocked()){ enforceFinalLockHard(true); }
  else if(localStorage.getItem(STORAGE_PREFIX + "accessGranted") === "1") prepareResumeGate();
});
